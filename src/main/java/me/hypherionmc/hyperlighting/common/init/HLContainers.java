...
public class HLContainers {
    ...
    public static final RegistryObject<MenuType<ContainerBatteryNeon>> BATTERY_NEON_CONTAINER = CONTAINERS.register("guibatteryneon", () -> IForgeContainerType.create((windowId, inv, data) -> {
        BlockPos pos = data.readBlockPos();
        Level world = inv.player.getCommandSenderWorld();
        return new ContainerBatteryNeon(windowId, world, pos, inv, inv.player);
    }));

    public static final RegistryObject<MenuType<ContainerSwitchBoard>> SWITCHBOARD_CONTAINER = CONTAINERS.register("guiswitchboard", () -> IForgeContainerType.create((windowId, inv, data) -> {
        BlockPos pos = data.readBlockPos();
        Level world = inv.player.getCommandSenderWorld();
