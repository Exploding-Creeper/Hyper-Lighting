...
public class FluidStackWidget extends AbstractWidget {
    ...
    public void renderToolTip(PoseStack pPoseStack, int pMouseX, int pMouseY) {
        if (isActive() && isHovered) {
            displayOn.renderTooltip(pPoseStack, Arrays.asList(LangUtils.getTooltipTitle("gui.fogger.fluidtooltipname").getVisualOrderText(), new TextComponent((int) (((float)this.getFluid.get().getFluidAmount() / this.getFluid.get().getCapacity()) * 100) + "%").getVisualOrderText()), pMouseX, pMouseY);
        }
    }
}
